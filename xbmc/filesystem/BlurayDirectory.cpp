                                       ...
                                       std::map<unsigned int, std::string>& playlist_langs) const
{
  ...
  for (const auto& title : allTitles)
  {
    ...
    {
      const unsigned int playlist{static_cast<unsigned int>(stoi(playlistMatch[0]))};
      ...
      else
      {
        ...
        pl.emplace_back(static_cast<unsigned int>(titleInfo->duration / 90000));
  ...
  std::ranges::sort(clips, [](const std::vector<unsigned int>& i,
                              const std::vector<unsigned int>& j) { return (i[0] < j[0]); });
