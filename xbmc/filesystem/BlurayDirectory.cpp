      ...
      for (const auto& commonPlaylist : commonStartingPlaylists)
      {
        ...
        for (const auto& playlist : playlists)
        {
          if (playlist[0] == commonPlaylist)
          {
            ...
            CLog::Log(LOGDEBUG, "Potential candidate group start at playlist {}", commonPlaylist);
            foundFirst = true;
          }
          ...
          {
            CLog::Log(LOGDEBUG, "Candidate playlist {} duration {}", commonPlaylist + episodeOffset,
                      duration);
            candidatePlaylists.emplace_back(playlist[0], 0);
            findIdenticalEpisodes = true;
          }
