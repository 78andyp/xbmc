...
std::string CBlurayDirectory::GetDiscInfoString(DiscInfo info)
{
  ...
  GetDiscInfo();
  if (!m_disc_info || !m_disc_info->bluray_detected)
    return "";
  ...
  switch (info)
  {
    case DiscInfo::TITLE:
    {
      ...
      title = m_disc_info->disc_name ? m_disc_info->disc_name : "";
#endif
...
#if (BLURAY_VERSION > BLURAY_VERSION_CODE(1, 0, 0))
      id = m_disc_info->udf_volume_id ? m_disc_info->udf_volume_id : "";
      if (id.empty())
        id = CDiscDirectoryHelper::HexToString(m_disc_info->disc_id, 10);
#endif
...
void CBlurayDirectory::GetPlaylists(ClipMap& clips, PlaylistMap& playlists) const
{
...
void CBlurayDirectory::GetTitles(GetTitlesJob job, CFileItemList& items, SortTitlesJob sort) const
{
  ...
  if (titleList.empty())
  {
    const int numTitles{GetNumberOfTitlesFromDisc()};
    for (int i = 0; i < numTitles; i++)
    {
      BLURAY_TITLE_INFO t{};
      if (!GetTitleInfoFromDisc(i, t))
...
bool CBlurayDirectory::InitializeBluray(const std::string& root)
{
  ...
  GetDiscInfo();
  m_blurayMenuSupport = m_disc_info && !m_disc_info->no_menu_support;
...
void CBlurayDirectory::GetDiscInfo()
{
  const std::string path{m_url.GetHostName()};
  const BLURAY_DISC_INFO* discInfo{};
  if (g_blurayDiscCache.GetDiscInfo(path, discInfo))
...
int CBlurayDirectory::GetNumberOfTitlesFromDisc() const
{
  return static_cast<int>(bd_get_titles(m_bd, TITLES_RELEVANT, 0));
}
                                               ...
                                               BLURAY_TITLE_INFO& playlistInfo) const
{
  // Check cache
  const std::string path{m_url.GetHostName()};
  if (g_blurayDiscCache.GetPlaylistInfo(path, playlist, playlistInfo))
    return true;
  ...
  // Cache and return
  g_blurayDiscCache.SetPlaylistInfo(path, playlist, p);
  playlistInfo = *p;

  return true;
}

bool CBlurayDirectory::GetTitleInfoFromDisc(unsigned int title, BLURAY_TITLE_INFO& titleInfo) const
{
  // Check cache
  const std::string path{m_url.GetHostName()};
  if (g_blurayDiscCache.GetTitleInfo(path, title, titleInfo))
    return true;

  // Retrieve from disc
  BLURAY_TITLE_INFO* t{bd_get_title_info(m_bd, title, 0)};
  if (!t)
    return false;

  // Cache and return
  g_blurayDiscCache.SetTitleInfo(path, title, t);
  titleInfo = *t;
