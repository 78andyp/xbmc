...
public:
  CGetDirectoryItems(std::string path, CFileItemList& items, CDirectory::CHints hints)
    : m_path(std::move(path)), m_items(items), m_hints(std::move(hints))
  {
  }
  ...
  std::ranges::copy_if(playlists, std::inserter(longPlaylists, longPlaylists.end()),
                       [&](const PlaylistMapEntry& p)
                       {
                         ...
                         return playlistInformation.duration >= MIN_EPISODE_DURATION &&
                                std::ranges::none_of(
                                    m_playAllPlaylists,
                                    [&playlist](const CandidatePlaylistInformation& p)
                                    { return playlist == p.playlist; });
                       });
                                                  ...
                                                  const PlaylistMap& playlists)
{
  ...
  if (m_candidatePlaylists.size() > 1 && m_isSpecial == IsSpecial::EPISODE)
  {
    ...
    auto oldCandidatePlaylists{m_candidatePlaylists};
    m_candidatePlaylists.clear();
    ...
    for (auto& playlist : oldCandidatePlaylists | std::views::values)
      indexes.insert(playlist.index);
    ...
    for (unsigned int currentEpisodeIndex : indexes)
    {
      ...
      std::ranges::sort(filteredCandidatePlaylists,
                  ...
                  [&](const PlaylistVectorEntry& playlist)
                  {
                    ...
                    const auto isEpisode{
                        std::ranges::any_of(m_candidatePlaylists | std::views::elements<0>,
                                            ...
                                            { return playlistNumber == candidatePlaylist; })};
                    const auto isPlayAll{std::ranges::any_of(
                        m_playAllPlaylists,
                        [&playlist](const CandidatePlaylistInformation& playAllPlaylist)
                        { return playlistNumber == playAllPlaylist.playlist; })};
                                             ...
                                             const PlaylistMap& playlists)
{
  ...
  if (m_isSpecial == IsSpecial::EPISODE)
  {
    ...
    for (const auto& playlist : sortedPlaylists)
    {
      ...
      GenerateItem(url, newItem, playlist.playlist, playlists, episodesOnDisc[playlist.index], IsSpecial::EPISODE);
      items.Add(newItem);
    }
  }
...
bool CDiscDirectoryHelper::GetOrShowPlaylistSelection(CFileItem& item)
{
  ...
  const int menuSelection{CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
      CSettings::SETTING_DISC_PLAYBACK)};

  if (!forceSelection && menuSelection == BD_PLAYBACK_DISC_MENU)
    return true; // No need to select playlist if disc menu will be shown
  if (forceSelection || menuSelection == BD_PLAYBACK_MAIN_TITLE ||
      menuSelection == BD_PLAYBACK_SIMPLE_MENU)
  {
    ...
    if (forceSelection || menuSelection == BD_PLAYBACK_SIMPLE_MENU)
    {
      while (true)
      {
        if (CGUIDialogSimpleMenu::ShowPlaylistSelection(item, selectedItem, items, usedPlaylists))
        {
          ...
          // Folder is all titles
          if (!GetItems(items,
                        selectedItem.GetDynPath())) // Get selected (usually all) titles
            return false;
        }
        else
          return false;
