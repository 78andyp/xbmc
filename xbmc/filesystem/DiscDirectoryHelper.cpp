...
bool CDiscDirectoryHelper::GetOrShowPlaylistSelection(CFileItem& item)
{
  ...
  ExcludeUsedPlaylists excludeUsedPlaylists{ExcludeUsedPlaylists::DONT_EXCLUDE_USED_PLAYLISTS};
      ...
      menuSelection == BD_PLAYBACK_SIMPLE_MENU)
  {
    ...
    if (excludeUsedPlaylists == ExcludeUsedPlaylists::EXCLUDE_USED_PLAYLISTS)
    {
      CVideoDatabase database;
      if (!database.Open())
      {
...
bool CDiscDirectoryHelper::GetItems(CFileItemList& items,
                                    const std::string& directory)
{
  items.Clear();
