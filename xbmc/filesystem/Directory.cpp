...
bool CDirectory::GetDirectory(const CURL& url, CFileItemList& items, const CHints& hints)
{
  ...
  if ((hints.flags & DIR_FLAG_FORCE_CACHE) == DIR_FLAG_FORCE_CACHE && hints.mask.empty() &&
      g_directoryCache.GetDirectory(realURL, items, true))
  {
    items.SetURL(url);
    PostProcessDirectory(url, items, hints);
    return true;
  }
...
void CDirectory::PostProcessDirectory(const CURL& url, CFileItemList& items, const CHints& hints)
{
  CURL realURL = URIUtils::SubstitutePath(url);
