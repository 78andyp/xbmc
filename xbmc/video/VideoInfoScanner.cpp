...
CVideoInfoScanner::~CVideoInfoScanner()
{
  ...
  long CVideoInfoScanner::AddVideo(CFileItem *pItem, const CONTENT_TYPE &content, bool videoFolder /* = false */, bool useLocal /* = true */, const CVideoInfoTag *showInfo /* = NULL */, bool libraryImport /* = false */)
  {
    ...
    if (movieDetails.m_iTrack > -1)
    {
      ...
      CURL url;
      if (URIUtils::IsDiscImage(movieDetails.m_basePath))
      {
        // If disc image, add udf://
        CURL url2;
        url2.SetHostName(movieDetails.m_basePath);
      ...
      url.SetFileName(URIUtils::AddFileToFolder(
          "BDMV", "PLAYLIST", StringUtils::Format("{:05}.mpls", movieDetails.m_iTrack)));
