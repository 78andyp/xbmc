...
bool HasPartNumberInTitle(const std::string& title)
{
  return GetPartNumberInTitle(title) != -1;
}

void RemovePartNumberFromTitleInternal(std::string& title)
{
  const int offset{GetPartNumberInTitle(title)};
  if (offset != -1)
    title.erase(offset);
}
} // namespace

void CGUIDialogVideoManagerVersions::RemovePartNumberFromTitle() const
{
  std::string& title{m_videoAsset->GetVideoInfoTag()->m_strTitle};
  if (HasPartNumberInTitle(title))
  {
    RemovePartNumberFromTitleInternal(title);
    m_database.SetMovieTitle(m_videoAsset->GetVideoInfoTag()->m_iDbId, title);
  }
}
